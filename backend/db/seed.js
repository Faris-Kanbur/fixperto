...
export function seedIfEmpty() {
  ...
  if (isEmpty("listings")) {
    const stmt = db.prepare(`INSERT INTO listings (id,sellerName,sellerType,brand,model,year,km,price,description,photo,status,px,py,offers,messages,fuelType,transmission,power,firstReg,color,vehicleId)
    VALUES (@id,@sellerName,@sellerType,@brand,@model,@year,@km,@price,@description,@photo,@status,@px,@py,@offers,@messages,@fuelType,@transmission,@power,@firstReg,@color,@vehicleId)`);
    const insertMany = db.transaction((rows) => { for (const l of rows) stmt.run({ ...l, offers: j(l.offers), messages: j(l.messages) }); });
    insertMany(LISTINGS);
  }
