...
| 8 | **E-posta gönderimini istek dışına al** — **Faz 5'te yapıldı** (süreç içi sıralı kuyruk, sınırlı, bir kez yeniden deneme) | **P2** | **Düşük** | **Ölçüldü** (2 sn gecikmeli SMTP): kayıt/giriş yanıtı **2006 ms → ~5 ms**. Kazanç girişte kritik: kayıt bir kez, giriş her gün yapılıyor. |
| 9 | **Kod bölme** — **Faz 6'da yapıldı, ama kapsam daraltıldı.** jspdf + jspdf-autotable dinamik `import()` (**ölçülen 391 KB / gzip ~128 KB**), el kitabı `React.lazy` (174 KB veri). **Yönetici panelinin tamamı BÖLÜNMEDİ:** ayrı bir bileşen değil, `AppShell.tsx`in (584 KB) içine gömülü — ayırmak büyük refactor olur ve denetimin kuralına aykırı. | **P2** | **Orta** | **ÖLÇÜLEMEYEN:** paket boyutu. Bu ortamda `vite build` çalışmıyor (rollup/esbuild ikilileri darwin-arm64, makine linux-aarch64; npm kapalı). Doğrulanan: statik bağımlılığın derlenmiş çıktıdan kalktığı, dinamik import'un çözüldüğü, PDF'in hâlâ üretildiği, Suspense sınırının var olduğu. |
...
| 14 | **Basit izleme** — **Faz 5'te yapıldı.** Rapor `/api/health`'e eklemeyi öneriyordu; **bu yanlıştı** ve değiştirildi: hata oranı, yavaş uçlar ve DB boyutu saldırı istihbaratı da, o yüzden `GET /api/admin/metrics` (yönetici arkasında). `/api/health` yalın kaldı. | **P2** | **Düşük** | Yavaşlama fark edilir hâle geldi. Sıfır bağımlılık, sıfır işletme maliyeti. |
