...
İkisi de "kod var, mantık doğru görünüyor, hiç hata fırlatmıyor" sınıfından — ama biri (slot kilidi) SADECE tek bir yazma yolunda çalışıyordu, diğeri (hız sınırı) HİÇBİR yazma yolunda çalışmıyordu, ikisi de yalnızca GERÇEKTEN İSTİSMAR EDİLMEYE ÇALIŞILARAK (doğrudan \`curl\` ile, arka arkaya gerçek istekler atılarak) ortaya çıktı. "Kontrol var" ile "kontrol çalışıyor" arasındaki fark, olumsuz senaryoyu bizzat tetiklemeden ölçülemez.`,
      ...
      {
        id: "randevu-fonksiyon-denetimi",
        title: "22.9 \"Randevuyla ilgili HER fonksiyon\" denetimi — üç bulgu daha",
        body: `Kullanıcı bir kez daha sordu: "hiç sorun yok mu?" — ve sonra "randevu alma istemini baştan sona kontrol et, randevu ile ilgili HER fonksiyonu". Bu, 22.7/22.8'de bulunanların TEK TİP bir hata sınıfı (optimistik güncelleme + geri alma yokluğu) olduğunu, ama o sınıfın appointments.js'e özgü olmadığını gösterdi — aynı desen frontend'in randevu durumu değiştiren fonksiyonlarında da vardı.

## 1) Altı durum fonksiyonu, sunucu yazması BAŞARISIZ olsa bile "başarılı" gösteriyordu
\`acceptAppt\`, \`rejectAppt\`, \`markNoShow\`, \`advanceStatus\`, \`cancelOwnAppt\`, \`confirmReschedule\` — hepsi aynı kalıbı tekrarlıyordu: yerel state'i İYİMSER olarak (sunucu yanıtını beklemeden) değiştir, toast'u ve \`fireNotification\`'ı HEMEN ateşle, \`persist()\`'e geri alma (\`rollback\`) fonksiyonu VERME. \`persist()\` yardımcı fonksiyonu geri almayı zaten destekliyordu (parametre orada duruyordu) ama hiçbiri kullanmıyordu. Sonuç: sunucu isteği 400/409 ile reddedilse bile ekran "Randevu kabul edildi ✅" gösteriyor, karşı tarafa "yeni durumunuz X" bildirimi gidiyordu — veritabanı hiç değişmemişken. Düzeltme: her altısı \`prevStatus\`'u (ve \`confirmReschedule\` için eski \`date\`/\`time\` çiftini) yakalayıp \`persist(..., rollback)\` ile geri alacak şekilde yeniden yazıldı; toast/SMS/bildirim adımları sunucu yanıtının \`.then()\`'ine taşındı — yani artık YALNIZCA gerçekten kaydedilince tetikleniyorlar.

## 2) \`submitReview\`: reddedilen bir yorum bile "yorum yapıldı" bayrağını KALICI olarak yakıyordu
\`reviewed\` alanı sunucuda tek yönlü (\`false→true\`, geri dönüşü yok — bkz. \`appointments.js\` PATCH). Eski kod \`api.mechanics.addReview(...)\`'u ateşliyor ama \`setAppointments(reviewed:true)\`, başarı toast'ı ve "yeni değerlendirme aldınız ⭐" bildirimini API yanıtı beklenmeden, KOŞULSUZ çalıştırıyordu. Sunucu yorumu reddetse bile (ör. zaten yorum yapılmış, ya da randevu owner'a ait değil) kullanıcı arayüzde "yorumunuz kaydedildi" görüyor ve \`reviewed:true\` kalıcı olarak yerel state'e yazılıyordu — bir sonraki oturumda sunucudan gerçek veri gelince kaybolsa da, o an gerçek bir yorum yazma şansı tükeniyordu (buton bir daha görünmüyordu). Düzeltme: tüm başarı adımları \`api.mechanics.addReview(...).then((res) => {...})\` içine taşındı; hata durumunda zaten var olan sebep-eşlemeli \`.catch()\` tek başına kalıyor.

## 3) \`completeApptWithWarranty\`: başarısız bir \`advanceStatus\` modalı SONSUZA KADAR açık bırakıyordu
Bu fonksiyon "Tamiri tamamla" modalında \`await advanceStatus(id)\` çağırıyordu. (1)'deki düzeltmeyle \`advanceStatus\` artık başarısız PATCH'te gerçekten reddediyor (öncesinde her zaman "başarılı" gibi davranıyordu) — ama burada \`try/catch\` yoktu, yani reddedilen bir \`await\` fonksiyonu ortasında durdurup \`completingApptId\`'yi hiç temizlemiyordu: kullanıcı "Kaydet"e bastığında hiçbir şey olmuyor, modal kapanmıyor, tekrar denemek için sayfayı yenilemekten başka çare kalmıyordu. Düzeltme: \`await\` bir \`try/catch\`e alındı; \`catch\` bloğu \`completingApptId\`/\`warrantyDaysForm\`'u temizleyip erken çıkıyor.

## 4) Yeniden planlama saat seçici, DOLU/GEÇMİŞ saatleri tıklanabilir gösteriyordu
Yeni randevu akışı (\`BookingCalendar.tsx\`) saatleri \`bookableSlots()\` ile alıyor — bu fonksiyon ham saat şablonunu (\`slotsForDate()\`) DÖNÜP her birine \`taken\`/\`past\` bayrağı ekliyor, dolu/geçmiş saatler devre dışı render ediliyor. Ama \`AppointmentCard.tsx\`'teki yeniden planlama saat seçici HAM \`slotsForDate()\` çağırıyordu — \`taken\`/\`past\` hiç hesaplanmıyor, her saat sıradan bir tıklanabilir düğme olarak çiziliyordu. Canlı testte doğrulandı: bir tamircinin bir saati zaten doluyken (aynı hesabın başka bir randevusu o saatte), yeniden planlama ekranında o saat SIRADAN bir düğme gibi görünüyor, seçilip kaydedilmeye çalışılıyordu — yalnızca 22.8'de eklenen backend 409 kilidi bunu gerçek çift rezervasyona dönüşmekten alıkoyuyordu. (Not: "dolu" bilgisi zaten yalnızca GÖRÜNTÜLEYEN kullanıcının kendi randevularından hesaplanıyor — bkz. \`BookingCalendar.tsx\`'teki "DÜRÜST SINIR" notu; başka bir kullanıcının aynı saati almış olması istemcide asla görünmez, oradaki gerçek koruma yine backend'in 409'udur.) Düzeltme: yeniden planlama saat seçici de \`bookableSlots()\` kullanacak şekilde değiştirildi; ayrıca \`bookableSlots()\`'a randevunun KENDİ id'sini hariç tutan isteğe bağlı bir \`excludeApptId\` parametresi eklendi — yoksa kişi kendi o anki saatini bile "dolu" görüp seçemezdi. Canlı doğrulama: aynı hesabın başka bir randevusuyla çakışan saat artık \`disabled\` + "Dolu" ipucuyla işaretleniyor; randevunun kendi mevcut saati ise normal seçilebilir kalıyor.

## Ders
Aynı optimistik-güncelleme hatası TEK bir yerde değil, randevu durumunu değiştiren HER fonksiyonda tekrarlanmış bir şablondu — birini bulup düzeltmek diğerlerini otomatik düzeltmiyor, her biri ayrı ayrı taranmalı. Ayrıca "aynı işi yapan iki fonksiyon" (burada \`slotsForDate\` / \`bookableSlots\`) varsa ve biri diğerinden EKSİK bir davranışa sahipse, hangi ekranın hangisini kullandığını kontrol etmeden "randevu akışı test edildi" denemez — iki ekran aynı özelliği farklı fonksiyonlarla uyguluyor olabilir.`,
      },
      {
        id: "randevu-fonksiyon-denetimi-2",
        title: "22.10 Aynı soruyu ÜÇÜNCÜ kez sormak zorunda kalmanın sebebi — dateISO ve üç bulgu daha",
        body: `22.9'daki "her fonksiyonu kontrol ettim" iddiası da EKSİK çıktı: kullanıcı haklı olarak "yine gözden kaçıracaksın" dedi, ve backend/routes/appointments.js'i bu kez SATIR SATIR (özet geçmeden) yeniden okumak üç YENİ bulgu çıkardı — üçü de önceki turlarda "kontrol edildi" denilen dosyalardaydı.

## 1) Yeniden planlama, randevunun \`dateISO\`'sunu HİÇ GÜNCELLEMİYORDU
\`date\`/\`time\` kullanıcıya gösterilen serbest metin ("27 Eylül" / "11:00"); \`dateISO\` ise GERÇEK, sıralanabilir tarih. Üç ayrı yer bunu okuyor: \`downloadAppointmentIcs\` (Takvime Ekle — .ics dosyasının GÜNÜNÜ buradan alıyor, saat ayrı \`time\` alanından), tamirci Analiz sekmesindeki tarih-aralığı süzgeci (\`withinDays\`, AppShell.tsx), ve en önemlisi \`routes/vehicleHistory.js\`'teki \`serviceDate: appt.dateISO || appt.date || ...\` — bu uygulamanın kendi yorumlarının "en güçlü güven sinyali" dediği DOĞRULANMIŞ SERVİS GEÇMİŞİ kaydının tarihi. \`confirmReschedule\` (AppLogicProvider.tsx) PATCH gövdesinde yalnızca \`{date, time}\` gönderiyordu; backend'in beyaz listesinde (\`OWNER_WRITABLE\`/\`MECHANIC_WRITABLE\`) \`dateISO\` de yoktu — yani ne istemci gönderiyordu ne sunucu kabul ediyordu. Sonuç: bir randevu ertelendiğinde veritabanındaki \`date\`/\`time\` doğru yeni günü gösteriyor ama \`dateISO\` İLK rezervasyonun tarihinde donuk kalıyordu. Canlı ölçüldü: 24 Eylül'e alınan bir randevu 27 Eylül'e ertelendi, \`date\`/\`time\` "27 Eylül"/"11:00" oldu ama \`dateISO\` "2026-09-24" olarak kaldı (düzeltmeden önce). Somut etkisi: ertelenen bir randevunun .ics dosyası YANLIŞ günü indiriyor (kullanıcı eski günde takvim hatırlatması alır, yeni randevu gününü kaçırabilir), tamamlanınca yazılan servis geçmişi kaydı da YANLIŞ tarihi taşıyordu. Düzeltme: \`dateISO\` beyaz listeye \`date\`/\`time\` ile birlikte eklendi, \`confirmReschedule\` artık \`confirmBooking\`'in randevu oluştururken yaptığı gibi (\`selectedDate.toISOString()\`) aynı deseni kullanıyor. Canlı doğrulama: erteleme sonrası \`dateISO\` artık yeni günü gösteriyor; \`dateISO\` göndermeyen eski bir istemci de (geriye dönük uyum) hâlâ sorunsuz çalışıyor.

## 2) Destek talebi üzerinden iade: "aldatılmış vekil" (confused deputy) açığı
\`issueTicketRefund\` (yönetici panelinde, bir talebi "iade et" ile çözerken) talebin \`relatedNote\` alanından regex ile bir randevu numarası çıkarıp o randevuyu DOĞRUDAN iptal edip iadeli işaretliyordu. \`relatedNote\` TALEBİ AÇAN KİŞİNİN serbest metnidir, sunucu hiç doğrulamaz — \`submitSupportTicket\`'ta kullanıcı bu alana istediği metni yazabilir. Yani kötü niyetli biri, kendi şikayetine "#<başkasının randevu numarası>" yazan bir destek talebi açıp, o randevunun hiç tarafı OLMASA bile yöneticiyi (metni satır satır karşılaştırmadığı bir anda) BAŞKA bir kullanıcının randevusunu iptal edip iade edilmiş göstermeye kandırabilirdi. Bu teknik bir yetki atlaması değil (yönetici zaten her randevuyu değiştirebilir) ama GERÇEK bir açık: sistemin normalde güvendiği bir aracı (yönetici), doğrulanmamış kullanıcı girdisiyle yanlış hedefe yönlendiriliyor. Aynı talette \`fromId\`/\`fromType\` alanları VAR ve bunlar (aksine) oturumdan damgalanıyor — kim açtığının tek güvenilir kaynağı bunlar. Düzeltme: iade artık yalnızca çözülen randevunun \`ownerId\`/\`mechanicId\`'si talebi açanın \`fromId\`/\`fromType\`'ıyla eşleşiyorsa uygulanıyor; eşleşmezse yöneticiye açık bir uyarı gösterilip randevuya dokunulmuyor.

## 3) Garanti süresi + hatırlatma yazma da aynı "geri alma yok" hatasını taşıyordu
\`completeApptWithWarranty\`, iş tamamlanırken opsiyonel garanti tarihini randevuya ve buna bağlı bir hatırlatmayı araca yazıyordu — ikisi de iyimser \`setState\` + \`persist()\` ama İKİSİ DE geri alma (\`rollback\`) fonksiyonu vermiyordu (22.9'da düzeltilen altı fonksiyonla AYNI kalıp, aynı dosyada, birkaç yüz satır ötede — 22.9'un taraması buraya uğramamıştı). PATCH başarısız olursa (ör. ağ hatası) ekranda garanti tarihi ve yeni hatırlatma "kaydedilmiş" görünmeye devam ediyor, sayfa yenilenince sessizce kayboluyordu. Düzeltme: her iki yazma da eski değeri (\`prevWarrantyEndDate\`, \`prevCustomReminders\`) yakalayıp \`persist(..., rollback)\` ile geri alacak şekilde güncellendi.

## 4) Farklı bir tamirciye geçince ÖNCEKİ tamircinin hizmet seçimi taşınabiliyordu
"Randevu Al" düğmesi (MechDetailBody.tsx, mekanik detay sayfası) sadece \`setScreen("booking")\` çağırıyordu — randevu taslağı state'ini (\`bookingService\`, \`selectedDate\`, \`selectedTime\`, \`selectedBookingVehicleId\`, \`approveExpensiveService\`, \`shareHistoryConsent\`) hiç temizlemiyordu. Yalnızca \`rebookAppt\` ve \`acceptQuoteOffer\` bu temizliği yapıyordu (ve \`rebookAppt\` da EKSİK yapıyordu — aşağıya bakın). Hizmet seçici, seçili hizmeti \`bookingService.name === s.name\` ile İSİM EŞLEŞTİREREK "seçili" gösteriyor (AppShell.tsx \`isSel\`). Canlı doğrulandı: "Usta Mehmet Oto Servis"te "Yağ Değişimi" (350₺) seçilip randevu TAMAMLANMADAN "Anadolu Servis"e geçildi — bu tamirci de "Yağ Değişimi" sunuyor ama 250₺'ye. Düzeltmeden önce "Randevu Al"a basınca hizmet seçici "Yağ Değişimi"yi hâlâ seçili gösterip özet panelinde 350₺ (Anadolu'nun DEĞİL, Usta Mehmet'in fiyatı) basardı — kullanıcı hiç fark etmeden Anadolu'ya, sunmadığı fiyattan bir randevu isteği gönderebilirdi. Tarih/saat güvenliydi (confirmBooking her seferinde \`bookableSlots(selectedMechanic, ...)\` ile YENİDEN doğruluyor) ama hizmet seçimi hiç yeniden doğrulanmıyordu. Düzeltme: paylaşılan tek bir \`startBooking()\` fonksiyonu — randevu taslağının TAMAMINI temizleyip ekranı açıyor; hem ana "Randevu Al" düğmesi hem \`rebookAppt\` artık bunu kullanıyor (rebookAppt önceden de \`selectedBookingVehicleId\`/\`approveExpensiveService\`/\`shareHistoryConsent\`'i unutuyordu — pahalı bir hizmet için işaretlenmiş "fiyatı onaylıyorum" kutusu bir SONRAKİ, alakasız randevuda sessizce onaylanmış sayılabiliyordu). Canlı doğrulandı: düzeltmeden sonra Anadolu Servis'in randevu ekranı "Hizmet: —" ile açılıyor, "Yağ Değişimi" kendi doğru fiyatıyla (250₺) ve SEÇİLİ OLMADAN görünüyor.

## 5) Çift tıklama koruması yoktu (küçük, ama backend'in neden gerçekten koruduğunu netleştiriyor)
"Randevuyu Onayla" düğmesi yalnızca \`ready\` (form dolu mu) ile devre dışı bırakılıyordu — istek SÜRERKEN değil. Hızlı çift tıklamada \`confirmBooking()\` iki kez, ilk \`await\` hiç bitmeden çağrılabiliyordu. Backend'deki slot kilidi (tek yazıcılı SQLite, tam senkron transaction — bkz. 22.8) ikinci isteği doğru şekilde 409 ile reddediyor, GERÇEK bir çift randevu oluşmuyor — ama kullanıcı ilk randevusu başarıyla oluşturulduğu anda hemen ardından kafa karıştırıcı bir hata tostu görüyordu. Basit bir \`bookingInFlightRef\` ile ikinci çağrı en baştan engellendi.

## 6) Tamircinin kendi çalışma saatlerini/otomatik onay ayarını düzenlemesi de aynı iki hatayı taşıyordu
Kullanıcı "randevu ile bağlantılı HERHANGİ bir yerde hata olmasın" dedi — bu, randevu EKRANININ kendisiyle sınırlı kalmayıp o ekranı BESLEYEN veriyi (tamircinin çalışma saatleri, otomatik kabul ayarı) de kapsamalıydı, çünkü ikisi de doğrudan \`bookableSlots\`/POST'un okuduğu alanlar. İkisinde de bulunan:
- **Güncelleyici içinde yan etki**: \`toggleDayOpen\`/\`toggleSlotClosed\`/\`addExtraSlot\`, \`persistMechanicHours(next)\` çağrısını \`setMechanicHours\`'un GÜNCELLEYİCİ FONKSİYONU İÇİNDE çalıştırıyordu — bu depoda \`toggleFavorite\`'in üstünde ZATEN belgelenmiş bir hata sınıfı: React StrictMode/eşzamanlı render'da güncelleyiciyi İKİ KEZ çağırabilir, yani PATCH iki kez gidebilir.
- **Geri alma yok**: \`persistMechanicHours\` ve \`updateMyField\` (otomatik kabul ayarı onun üzerinden yazılıyordu) hiçbir zaman rollback almıyordu. PATCH başarısız olursa tamircinin ekranı "kaydettim" gösteriyor, sunucudaki gerçek değer DEĞİŞMEMİŞ kalıyordu — \`hoursText\` alanı yalnızca tamircinin kendi ekranını değil, DİĞER TÜM kullanıcıların o tamirci için gördüğü randevu saatlerini belirlediği için (bkz. madde 1'deki \`slotsFromHoursText\`), tamirci "Pazar'ı kapattım" sanırken müşteriler sunucudaki eski (açık) saatle randevu almaya devam edebilirdi. Otomatik kabul ayarı için de aynısı: "randevularımı ben onaylayacağım" dediğini sanan bir tamircinin randevuları sessizce otomatik kabul edilmeye devam edebilirdi.
Düzeltme: her üç saat fonksiyonu \`next\`i updater'ın DIŞINDA hesaplayıp \`setMechanicHours(next)\`i DOĞRUDAN çağırıyor; \`persistMechanicHours\` artık eski değeri geri yükleyen bir rollback alıyor. \`toggleAutoAccept\` genel amaçlı \`updateMyField\`'ı (metin alanları için KASITLI olarak rollback'siz — kullanıcı yazarken geri almak rahatsız edici olurdu) kullanmayı bırakıp kendi ayrık rollback'ini aldı. Canlı doğrulandı: backend kapalıyken hem "Pazar'ı aç" hem "otomatik onayı kapat" denendi, ikisi de PATCH başarısız olunca ekranda doğru şekilde ESKİ değerine geri döndü (DOM'da anahtar/toggle rengi kontrol edilerek); backend açıkken aynı işlemler veritabanına doğru yazıldı (sqlite3 ile doğrudan kontrol edildi).

## Ders — neden bu kadar çok tur gerekti
İlk iki turda (22.7/22.8) bulunanlar GERÇEKTEN düzeltildi ve testlerle doğrulandı — ama "randevu akışını kontrol ettim" her seferinde YANLIŞ bir kapsamda söylendi: bir tur yalnızca durum-geçişi bug sınıfını taradı, bir sonraki yalnızca slot kilidi/hız sınırlayıcılarını, bir sonraki yalnızca optimistik-güncelleme kalıbını. Her tur kendi bulduğu sınıf için doğruydu, ama "her fonksiyon" iddiası bunu YANLIŞLIKLA "artık hiçbir sınıfta sorun kalmadı"ya genişletiyordu. Bu turda fark eden şey satır satır TAM DOSYA okuması (özetle değil) ve her alanın NEREDE OKUNDUĞUNU aşağı doğru takip etmekti (\`dateISO\` üç ayrı dosyada okunuyordu, ikisi appointments.js'in kendisiyle hiç ilgisi yokmuş gibi duran vehicleHistory.js ve AppShell.tsx analiz sekmesiydi; \`bookingService\` de aynı şekilde — "randevu al" ekranının GİRİŞ noktasında hiç reset yoktu, bu da yalnızca ekranın kendisini değil, oraya GİREN düğmeleri de taramayı gerektirdi). Bir alanın "yazılabilir" olması onun DOĞRU yazıldığı anlamına gelmez; bir alanın nerede TÜKETİLDİĞİNE ve state'in NEREDE BAŞLATILDIĞINA bakmadan "bu iş bitti" denemez.`,
      },
      {
        id: "sohbet-tam-denetim",
        title: "22.11 Tamirci ↔ araç sahibi sohbetinin tam denetimi — bir kimlik sahteciliği açığı, eksik hız sınırı",
        body: `Randevu/bildirim taramaları tükenince kullanıcı sohbete geçti: "tamirci ve araç sahibi arasındaki sohbete bak, onla ilgili herşeyi kontrol et". \`backend/routes/conversations.js\` daha önce ciddi bir güvenlik denetiminden geçmiş (mesaj sahteciliği, boyut tavanları, sahiplik kontrolü — dosyanın üst kısmındaki büyük yorumlar) ama bu turda satır satır yeniden okumak iki YENİ, gerçek bulgu çıkardı.

## 1) PATCH ile kimlik sahteciliği — POST'taki korumanın YARISI eksikti
Dosyanın kendi yorumu POST için şunu anlatıyor: \`mechanicName\`/\`mechanicImg\`/\`mechanicLang\` (sohbet listesinde gösterilen karşı taraf kopyaları) istemciden alınmıyor, kaynak owner/mechanic kaydından okunuyor — "biri kendini başka biri gibi gösteremesin" diye. Ama bu koruma yalnızca sohbet OLUŞTURULURKEN (POST) uygulanmıştı; PATCH uç noktası \`messages\` alanını (ayrı bir nedenle, mesaj sahteciliğine karşı) admin'e kilitliyordu ama bu üç kimlik-kopyası alanını hiç kısıtlamıyordu. Canlı ölçüldü: gerçek bir sohbet açılıp \`PATCH {mechanicName:"SAHTE AD"}\` gönderildi — \`200\` döndü ve kalıcı olarak yazıldı. Yani bir sohbetin TARAFLARINDAN biri, karşı tarafın o sohbette görünen adını/fotoğrafını istediği gibi değiştirebiliyordu — POST'un önlemek için var olduğu tam olarak aynı oltalama riski, farklı bir HTTP yoluyla (appointments.js'teki DELETE/PATCH bypass'larıyla AYNI ders: bir alanı koruyan kural tek bir yazma yoluna konursa diğer yollar sessiz bir bypass olur). Düzeltme: bu üç alan da artık \`messages\` ile aynı muameleyi görüyor — PATCH'te yalnızca admin değiştirebilir. Canlı doğrulandı: aynı istek artık \`403\` + "Bu alanları değiştirme yetkiniz yok" dönüyor; \`pendingContextNote\` gibi meşru PATCH alanları hâlâ \`200\` ile çalışıyor (regresyon yok).

## 2) Sitede EN yüksek frekanslı yazma eylemi, tek hız sınırı OLMAYAN dosyaydı
appointments.js, notifications.js, reviews.js, vehicleHistory.js — hepsinde \`makeRateLimiter\` var. conversations.js'te SIFIR: ne mesaj gönderme (\`POST /:id/messages\`), ne sohbet oluşturma (\`POST /\`), ne silme. Mesaj gönderme özellikle riskli çünkü hem sitedeki en sık tekrarlanan meşru eylem hem de her istekte mevcut mesaj dizisinin TAMAMI okunup (\`JSON.parse\`) yeniden yazılıyor (\`JSON.stringify\`) — bkz. dosyanın kendi performans notu, 40 MB'a kadar büyüyebilen bir sütun. Kimliği doğrulanmış tek bir hesap saniyede onlarca istekle hem karşı tarafın sohbetini spam'le doldurabilir hem de sunucuyu büyüyen bir diziyi tekrar tekrar serileştirmeye zorlayabilirdi. Düzeltme: appointments.js'teki \`APPOINTMENT_CREATE_LIMIT_PER_WINDOW\` deseniyle birebir aynı şekilde, env değişkeniyle geçilebilir iki sınırlayıcı eklendi (mesaj: 60/dakika, sohbet oluşturma: 20/10dakika) ve e2e test harness'ine (\`tests/e2e/harness.mjs\`) aynı geçersiz kılma deseni eklendi. Canlı doğrulandı: ayrı bir portta düşük bir eşikle (3/dakika) başlatılan geçici bir sunucuda ilk 3 mesaj \`200\`, 4. ve 5. istek doğru şekilde \`429\` döndü.

## 3) Mesaj gönderme fonksiyonları, randevu tarafında zaten bulunup düzeltilen İKİ hatayı taşıyordu
\`sendOwnerMessage\`/\`sendMechMessage\`: \`fireNotification\` sunucu yanıtı hiç BEKLENMEDEN, mesaj eklenirken \`api.conversations.appendMessages(...)\` çağrısının hemen ardından koşulsuz ateşleniyordu — istek başarısız olsa bile (hız sınırı, ağ hatası) karşı taraf HİÇ GÖNDERİLMEMİŞ bir mesaj için bildirim alıyordu. Ayrıca başarısızlıkta yerel iyimser mesaj balonu hiç geri alınmıyordu: kullanıcı "⚠️ kaydedilemedi" tostu görüyor ama mesaj sohbette gönderilmiş gibi görünmeye devam ediyordu. Düzeltme: bildirim artık yalnızca \`.then()\` içinde (gerçek başarıda); \`.catch()\` artık mesajı eski hâline (\`prevMessages\`) geri alıyor. \`removeConversation\` de aynı sınıf eksik geri almayı taşıyordu (silme başarısız olursa sohbet yerelde kaybolmuş gibi görünüp sayfa yenilenince geri geliyordu) — düzeltildi. Sohbet OLUŞTURMA fonksiyonlarının üçü de (\`openChatWithMechanic\`, \`openChatWithOwner\`, \`openMechChatWithOwnerListing\`) aynı eksikliği taşıyordu: oluşturma başarısız olursa kullanıcı zaten "hayalet" bir sohbete yönlendiriliyor, boş ekranda mesaj yazmaya çalışınca ancak dolaylı bir hatayla sorunu fark ediyordu — üçüne de rollback eklendi (hayalet sohbeti listeden kaldırıp aktif sohbet göstergesini temizliyor).

## Ders
Bu dosya daha önce GERÇEKTEN ciddi bir denetimden geçmişti (üstteki uzun güvenlik yorumları bunu kanıtlıyor) — ama o denetim bile bir alanı SADECE BİR yazma yolunda (POST) korumuş, PATCH'i unutmuştu; ve o denetim hız sınırlarını hiç kapsamamıştı, çünkü o zaman appointments.js/notifications.js'teki DoS deseni henüz bu oturumda keşfedilmemişti. "Bu dosya zaten denetlendi" bir dosyayı tekrar okumamak için gerekçe değildir — her denetim kendi bulduğu sınıfla sınırlıdır, sonraki bir denetimde ortaya çıkan YENİ bir sınıf (burada: hız sınırı eksikliği) eski, "tamamlanmış" denetimlerin gözden kaçırdığı gerçek bir açığı hâlâ barındırabilir.`,
      },
    ],
  },
  {
