    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto mt-4">
        <div className="px-5 md:px-8">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-3 mb-5 space-y-1">{(selectedMechanic.id === MY_MECHANIC_ID ? formatHoursText(mechanicHours, lang) : selectedMechanic.hoursText || []).map((line, i) => { const isClosed = line.includes("Kapalı") || line.includes("Closed") || line.includes("Geschlossen"); return (<div key={i} className="flex justify-between text-xs"><span className="text-gray-500">{line.split(":")[0]}</span><span className={isClosed ? "text-red-400" : "text-gray-700 font-medium"}>{line.split(/:(.+)/)[1]}</span></div>); })}</div>
