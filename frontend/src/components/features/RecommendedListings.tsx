  ...
  return (
    <section className="mb-6" aria-label={t("recommendedTitle")}>
      <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
        <h2 className="font-bold text-fg text-base flex items-center gap-2">
          <Sparkles size={16} className="text-info" /> {t("recommendedTitle")}
        </h2>
        ...
        {!recommendations.consent && (
          <button onClick={openOwnerSettingsForRecs}
            className="inline-flex items-center gap-1.5 text-[11px] text-fg-secondary hover:text-primary transition">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {recommendations.listings.map((l) => (
          <div key={l.id}>
            <ListingCard l={l} />
            <p className="text-[11px] text-fg-muted mt-1.5 px-1 leading-snug">{reasonText(l.recommendReason)}</p>
          </div>
        ))}
      </div>
    </section>
