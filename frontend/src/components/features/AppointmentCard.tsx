  ...
  const {
    ...
    return (
      <div className="border border-surface-elevated rounded-2xl p-4 shadow-sm">
        ...
        {cancellable && !isRescheduling && (<div className="flex gap-2 mt-3"><button onClick={() => startReschedule(a)} className="flex-1 border border-blue-200 text-primary text-xs py-2 rounded-xl font-medium hover:bg-primary-tint transition">{t("reschedule")}</button><button onClick={() => setConfirmDialog({ title: t("cancelApptConfirmTitle"), body: t("cancelApptConfirmBody"), confirmLabel: t("cancelApptConfirmLabel"), danger: true, onConfirm: () => cancelOwnAppt(a.id) })} className="flex-1 border border-red-200 text-red-500 text-xs py-2 rounded-xl font-medium hover:bg-error-tint transition">{t("cancelAppt")}</button></div>)}
        {isRescheduling && (() => {
