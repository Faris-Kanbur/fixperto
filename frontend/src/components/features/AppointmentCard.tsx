...
export function AppointmentCard({ a }) {
  const {
    ...
    return (
      <div className="border border-surface-elevated rounded-2xl p-4 shadow-sm">
        <div className="flex items-start gap-3 mb-3"><div className="text-2xl bg-primary-tint rounded-xl w-11 h-11 flex items-center justify-center flex-shrink-0">{a.mechanicImg}</div><div className="flex-1"><h4 className="font-semibold text-fg-strong text-sm">{a.mechanicName}</h4><p className="text-xs text-fg-muted">{a.vehicle}</p></div><span className={`text-[10px] px-2 py-1 rounded-full font-medium whitespace-nowrap ${statusColor(a.status)}`}>{apptStatusLabel(a.status, lang)}</span></div>
        <p className="text-xs text-fg-secondary mb-3">{a.issue}</p>
        {a.issuePhotos && a.issuePhotos.length > 0 && (<div className="flex gap-1.5 mb-3">{a.issuePhotos.map((src, i) => (<img loading="lazy" decoding="async" key={i} src={src} alt={`${a.vehicle || ""} arıza fotoğrafı ${i + 1}`} className="w-12 h-12 rounded-lg object-cover border border-surface-elevated" />))}</div>)}
        <div className="flex items-center gap-3 text-xs text-fg-muted mb-3"><span className="flex items-center gap-1"><Calendar size={12} />{a.date}</span><span className="flex items-center gap-1"><Clock size={12} />{a.time}</span></div>
        {cancellable && (
          <div className="flex items-center gap-3 text-[11px] text-info mb-3">
        ...
        {a.depositPaid > 0 && (<p className="text-[11px] text-success mt-2 flex items-center gap-1"><CheckCircle2 size={11} /> {t("depositPaidDemoNote", { amount: String(a.depositPaid) })}</p>)}
        {a.paymentMethod === "onsite" && (<p className="text-[11px] text-fg-muted mt-2 flex items-center gap-1"><Banknote size={11} /> {t("paymentOnsiteNote")}</p>)}
        {a.warrantyEndDate && (<p className="text-[11px] text-fg-secondary mt-2 flex items-center gap-1"><Shield size={11} className="text-success flex-shrink-0" /> {t("warrantyUntilNote", { date: new Date(a.warrantyEndDate).toLocaleDateString("tr-TR") })}</p>)}
        {a.status === "Tamir Tamamlandı" && !a.reviewed && (<button onClick={() => { setReviewingApptId(a.id); setReviewForm({ rating: 5, comment: "" }); }} className="w-full mt-3 border border-fg-muted text-fg-strong text-xs py-2 rounded-xl font-medium hover:bg-surface-elevated transition flex items-center justify-center gap-1"><Star size={12} /> {t("writeReviewTitle")}</button>)}
        {a.status === "Tamir Tamamlandı" && a.reviewed && (<p className="mt-3 text-[11px] text-success flex items-center gap-1 justify-center"><CheckCircle2 size={12} /> {t("reviewSubmittedNotice")}</p>)}
        {a.status === "Tamir Tamamlandı" && (<div className="flex gap-2 mt-2"><button onClick={() => rebookAppt(a)} className="flex-1 bg-primary-tint text-primary text-xs py-2 rounded-xl font-medium hover:bg-primary-tint transition flex items-center justify-center gap-1"><Calendar size={12} /> {t("rebookApptBtn")}</button><button onClick={() => downloadAppointmentReceipt(a)} className="flex-1 border border-border text-fg-secondary text-xs py-2 rounded-xl font-medium hover:bg-background transition flex items-center justify-center gap-1"><FileText size={12} /> {t("downloadReceiptBtn")}</button></div>)}
        {cancellable && !isRescheduling && (<div className="flex gap-2 mt-3"><button onClick={() => startReschedule(a)} className="flex-1 border border-primary-subtle text-primary text-xs py-2 rounded-xl font-medium hover:bg-primary-tint transition">{t("reschedule")}</button><button onClick={() => setConfirmDialog({ title: t("cancelApptConfirmTitle"), body: t("cancelApptConfirmBody"), confirmLabel: t("cancelApptConfirmLabel"), danger: true, onConfirm: () => cancelOwnAppt(a.id) })} className="flex-1 border border-error-tint text-error text-xs py-2 rounded-xl font-medium hover:bg-error-tint transition">{t("cancelAppt")}</button></div>)}
        {isRescheduling && (() => {
