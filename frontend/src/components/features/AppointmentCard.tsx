  ...
  const {
    ...
    return (
      <div className="border border-surface-elevated rounded-2xl p-4 shadow-sm">
        ...
        {a.warrantyEndDate && (<p className="text-[11px] text-fg-secondary mt-2 flex items-center gap-1"><Shield size={11} className="text-green-500 flex-shrink-0" /> {t("warrantyUntilNote", { date: new Date(a.warrantyEndDate).toLocaleDateString("tr-TR") })}</p>)}
        {a.status === "Tamir Tamamlandı" && !a.reviewed && (<button onClick={() => { setReviewingApptId(a.id); setReviewForm({ rating: 5, comment: "" }); }} className="w-full mt-3 border border-border text-fg-strong text-xs py-2 rounded-xl font-medium hover:bg-surface-elevated transition flex items-center justify-center gap-1"><Star size={12} /> {t("writeReviewTitle")}</button>)}
        ...
        {a.status === "Tamir Tamamlandı" && (<div className="flex gap-2 mt-2"><button onClick={() => rebookAppt(a)} className="flex-1 bg-primary-tint text-primary text-xs py-2 rounded-xl font-medium hover:bg-blue-100 transition flex items-center justify-center gap-1"><Calendar size={12} /> {t("rebookApptBtn")}</button><button onClick={() => downloadAppointmentReceipt(a)} className="flex-1 border border-border text-fg-secondary text-xs py-2 rounded-xl font-medium hover:bg-background transition flex items-center justify-center gap-1"><FileText size={12} /> {t("downloadReceiptBtn")}</button></div>)}
        {cancellable && !isRescheduling && (<div className="flex gap-2 mt-3"><button onClick={() => startReschedule(a)} className="flex-1 border border-primary-subtle text-primary text-xs py-2 rounded-xl font-medium hover:bg-primary-tint transition">{t("reschedule")}</button><button onClick={() => setConfirmDialog({ title: t("cancelApptConfirmTitle"), body: t("cancelApptConfirmBody"), confirmLabel: t("cancelApptConfirmLabel"), danger: true, onConfirm: () => cancelOwnAppt(a.id) })} className="flex-1 border border-red-200 text-red-500 text-xs py-2 rounded-xl font-medium hover:bg-error-tint transition">{t("cancelAppt")}</button></div>)}
        {isRescheduling && (() => {
