  ...
  const {
    ...
    return (
      <div className="border border-gray-100 rounded-2xl p-4 shadow-sm">
        ...
        {cancellable && (
          <div className="flex items-center gap-3 text-[11px] text-blue-500 mb-3">
        ...
        {a.status === "Tamir Tamamlandı" && (<div className="flex gap-2 mt-2"><button onClick={() => rebookAppt(a)} className="flex-1 bg-blue-50 text-blue-600 text-xs py-2 rounded-xl font-medium hover:bg-blue-100 transition flex items-center justify-center gap-1"><Calendar size={12} /> {t("rebookApptBtn")}</button><button onClick={() => downloadAppointmentReceipt(a)} className="flex-1 border border-gray-200 text-gray-600 text-xs py-2 rounded-xl font-medium hover:bg-gray-50 transition flex items-center justify-center gap-1"><FileText size={12} /> {t("downloadReceiptBtn")}</button></div>)}
        {cancellable && !isRescheduling && (<div className="flex gap-2 mt-3"><button onClick={() => startReschedule(a)} className="flex-1 border border-blue-200 text-blue-600 text-xs py-2 rounded-xl font-medium hover:bg-blue-50 transition">{t("reschedule")}</button><button onClick={() => setConfirmDialog({ title: t("cancelApptConfirmTitle"), body: t("cancelApptConfirmBody"), confirmLabel: t("cancelApptConfirmLabel"), danger: true, onConfirm: () => cancelOwnAppt(a.id) })} className="flex-1 border border-red-200 text-red-500 text-xs py-2 rounded-xl font-medium hover:bg-red-50 transition">{t("cancelAppt")}</button></div>)}
        {isRescheduling && (() => {
          ...
          <div className="mt-3 bg-white border border-gray-200 rounded-xl p-3">
            <div className="flex gap-2 mb-2 overflow-x-auto pb-1">{nextDays.map((d, i) => { const isSel = rescheduleDate?.toDateString() === d.toDateString(); const open = isDayOpenForMechanic(mech, d); return (<button key={i} disabled={!open} onClick={() => setRescheduleDate(d)} className={`flex-shrink-0 w-12 py-1.5 rounded-lg border text-center transition ${!open ? "opacity-30 cursor-not-allowed" : isSel ? "bg-blue-600 border-blue-600 text-white" : "border-gray-200 text-gray-600"}`}><p className="text-[9px]">{d.toLocaleDateString("tr-TR", { weekday: "short" })}</p><p className="text-xs font-bold">{d.getDate()}</p></button>); })}</div>
            {rescheduleDate && (<div className="grid grid-cols-4 gap-1.5 mb-2">{slotsForDate(mech, rescheduleDate).map(tm => (<button key={tm} onClick={() => setRescheduleTime(tm)} className={`py-1.5 rounded-lg border text-[11px] font-medium transition ${rescheduleTime === tm ? "bg-blue-600 border-blue-600 text-white" : "border-gray-200 text-gray-600"}`}>{tm}</button>))}{slotsForDate(mech, rescheduleDate).length === 0 && <p className="col-span-4 text-[11px] text-gray-400 text-center py-2">{t("bookingClosedDay")}</p>}</div>)}
            <div className="flex gap-2"><button onClick={() => setReschedulingApptId(null)} className="flex-1 text-xs py-2 rounded-lg border border-gray-200 text-gray-500">{t("cancel")}</button><button disabled={!rescheduleDate || !rescheduleTime} onClick={confirmReschedule} className="flex-1 text-xs py-2 rounded-lg bg-blue-600 text-white disabled:opacity-40">{t("save")}</button></div>
          </div>
        ); })()}
      </div>
    );
