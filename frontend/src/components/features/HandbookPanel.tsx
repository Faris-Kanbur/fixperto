...
function renderBody(body: string) {
  return String(body || "").split(/\n{2,}/).filter((b) => b.trim()).map((block, i) => {
    ...
    if (line.startsWith("## ")) {
      return <h3 key={i} className="text-sm font-bold text-fg mt-6 mb-2 flex items-center gap-2">
        <span className="w-1 h-4 bg-info rounded-full flex-shrink-0" />{line.slice(3)}
      </h3>;
    }
    return <p key={i} className="text-[13px] leading-relaxed text-fg-secondary mb-3 whitespace-pre-line">{line}</p>;
  });
}
...
export function HandbookPanel() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5 items-start">
      {/* ---- SOL: arama + ağaç ---- */}
      <div className="bg-white border border-border rounded-2xl overflow-hidden lg:sticky lg:top-4">
          ...
          <div className="relative">
            ...
            {query && (
              <button onClick={() => setQuery("")} aria-label="Aramayı temizle"
                className="absolute right-2 top-1/2 -translate-y-1/2 text-fg-muted hover:text-fg-secondary"><X size={13} /></button>
            )}
          </div>
        </div>
        ...
        <div className="max-h-[70vh] overflow-y-auto p-2">
          {matches ? (
            matches.length === 0 ? (
              <p className="text-xs text-fg-muted px-3 py-6 text-center">Eşleşme yok.</p>
            ) : (
              <>
                <p className="text-[10px] uppercase tracking-wide text-fg-muted px-3 py-2">{matches.length} sonuç</p>
                {matches.map((p) => (
                  <button key={p.id} onClick={() => { openPage(p); setQuery(""); }}
                    className="w-full text-left px-3 py-2 rounded-lg hover:bg-background transition">
                    <span className="block text-xs font-semibold text-fg-strong">{p.title}</span>
                    <span className="block text-[10px] text-fg-muted truncate">{p.sectionTitle}</span>
                  </button>
                ))}
              </>
            )
              ...
              return (
                <div key={s.id} className="mb-0.5">
                  <button onClick={() => toggle(s.id)} aria-expanded={open}
                    className="w-full flex items-center gap-1.5 px-2 py-2 rounded-lg text-left hover:bg-background transition">
                    {open ? <ChevronDown size={13} className="text-fg-muted flex-shrink-0" /> : <ChevronRight size={13} className="text-fg-muted flex-shrink-0" />}
                    <span className="text-xs font-bold text-fg-strong min-w-0 truncate">{s.title}</span>
                  </button>
                  {open && (
                    <div className="ml-4 border-l border-surface-elevated pl-2">
                      {s.pages.map((p) => {
                        const on = activeId === p.id;
                        return (
                          <button key={p.id} onClick={() => setActiveId(p.id)}
                            className={`w-full text-left px-2 py-1.5 rounded-lg text-xs transition flex items-center gap-1.5 ${on ? "bg-primary-tint text-primary-hover font-semibold" : "text-fg-secondary hover:bg-background"}`}>
      ...
      {/* ---- SAĞ: sayfa içeriği ---- */}
      <div className="bg-white border border-border rounded-2xl p-5 md:p-8 min-h-[60vh]">
        <div className="flex items-center gap-2 text-[11px] text-fg-muted mb-1">
          <BookOpen size={12} /> {active.sectionTitle}
        </div>
        <h2 className="text-xl md:text-2xl font-bold text-fg mb-5">{active.title}</h2>
