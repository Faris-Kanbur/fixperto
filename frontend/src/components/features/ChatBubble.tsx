...
export function ChatBubble({ msg, viewerLang, mine }) {
  const {
    ...
    return (<div className={`flex ${mine ? "justify-end" : "justify-start"} mb-3`}><div className={`max-w-[75%] rounded-2xl px-3 py-2 ${mine ? "bg-blue-600 text-white rounded-br-sm" : "bg-gray-100 text-gray-800 rounded-bl-sm"}`}>{msg.image && <img loading="lazy" decoding="async" src={msg.image} alt="upload" className="rounded-xl mb-1 max-h-40 object-cover" />}{msg.text && <p className="text-sm whitespace-pre-line">{displayText}</p>}{needsTranslation && msg.text && (<button onClick={() => toggleTranslate(msg.id)} className={`flex items-center gap-1 text-[10px] mt-1 ${mine ? "text-blue-100" : "text-gray-400"} hover:underline`}>{isTranslating ? <Loader2 size={10} className="animate-spin" /> : <Globe size={10} />} {isTranslating ? "Çevriliyor…" : showTr ? "Orijinali gör" : "Çeviriyi gör"}</button>)}</div></div>);
  }
