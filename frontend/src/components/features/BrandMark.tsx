...
export function BrandMark({ size = "md", className = "" }: { size?: "sm" | "md"; className?: string }) {
  ...
  return (
    ...
    >
      <div className={`${box} bg-primary rounded-lg flex items-center justify-center`}><Wrench size={icon} className="text-white" /></div>
      <span className={`${text} font-extrabold tracking-tight text-fg`}>Fix<span className="text-primary">perto</span></span>
    </button>
  );
}
...
export function PageTopBar({ onBack = null, right = null }: { onBack?: (() => void) | null; right?: React.ReactNode }) {
  ...
  return (
     ...
     */
    <div className="sticky top-0 z-[45] bg-white/95 backdrop-blur border-b border-surface-elevated">
      <div className="max-w-7xl mx-auto px-5 md:px-8 h-14 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          {onBack && (
            <button onClick={onBack} aria-label={t("back")} className="w-9 h-9 -ml-1 rounded-full hover:bg-surface-elevated flex items-center justify-center text-fg-secondary transition flex-shrink-0">
