...
export function PhotoLightbox({ photos, index, onIndexChange, onClose, title = "" }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col" role="dialog" aria-modal="true" aria-label={t("photoGalleryAria")}>
      <div className="flex items-center justify-between px-4 py-3 text-white/90 flex-shrink-0">
        ...
        <button onClick={onClose} aria-label={t("closeAria")} className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition"><X size={18} /></button>
      </div>
      <div className="flex-1 relative flex items-center justify-center min-h-0 px-4">
        {count > 1 && (
          <button onClick={() => onIndexChange((safeIndex - 1 + count) % count)} aria-label={t("prevPhotoAria")} className="absolute left-2 sm:left-6 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition z-10"><ChevronLeft size={22} /></button>
        )}
        ...
        {count > 1 && (
          <button onClick={() => onIndexChange((safeIndex + 1) % count)} aria-label={t("nextPhotoAria")} className="absolute right-2 sm:right-6 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition z-10"><ChevronRight size={22} /></button>
        )}
      </div>
      {count > 1 && (
